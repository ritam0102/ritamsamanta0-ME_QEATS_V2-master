...
class RestaurantServiceTest {
  ...
  void normalHourServingRadiusIs5Kms() throws IOException {
    ...
    // 2. Mock the repository service
    when(restaurantRepositoryServiceMock
        .findAllRestaurantsCloseBy(any(Double.class), any(Double.class), any(LocalTime.class),
            any(Double.class)))
        .thenReturn(restaurants);

    // 3. Call the service at a non-peak hour (e.g., 11:00 AM)
    GetRestaurantsResponse response = restaurantService
        .findAllRestaurantsCloseBy(new GetRestaurantsRequest(20.0, 30.0), LocalTime.of(11, 0));

    // 4. Verification & Assertions
    // Ensure the response size matches your JSON fixture (usually 3 for normal hours)
    ...
    // This is the core logic: Normal hours must use 5.0km
    assertEquals(5.0, servingRadiusInKms.getValue(), 0.01);
  }
